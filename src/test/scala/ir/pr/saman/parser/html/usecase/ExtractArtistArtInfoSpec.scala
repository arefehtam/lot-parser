...
class ExtractArtistArtInfoSpec extends WordSpec {
  ...
  "parser" should {
    "extract the same artist name" in {
      val names = ExtractArtistArtInfoUseCase.call(Constants.ARTIST) map (f => f.info getOrElse(Constants.ARTIST, "")) filter (_ == artistName)
      assert(names.headOption.getOrElse("") === artistName)
    }

    "contains specified title" in {
      val works = ExtractArtistArtInfoUseCase.call(Constants.ARTIST) map (f => f.info getOrElse(Constants.WORKS, ""))
